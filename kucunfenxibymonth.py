...
def load_and_preprocess_data_from_df(df):
    try:
        ...
        # 13. 预计总库存需要消耗天数
        df["预计总库存需要消耗天数"] = (
                (df["预计总库存用完"] - df["时间"]).dt.total_seconds() / (24 * 3600)
        ).round().astype(int)
        ...
        days_available = (TARGET_DATE - df["时间"]).dt.days
...
def get_week_data(df, target_date):
    ...
    week_data = df[df["时间"] == target_date].copy()
    # 注意：这里已经删掉了剔除非年份品的行！
...
def render_status_change_table(data, page=1, page_size=30):
    ...
    # 日期列格式化
    date_cols = ["时间", "预计FBA+AWD+在途用完时间", "预计总库存用完"]
    for col in date_cols:
        if col in paginated_data.columns:
            paginated_data[col] = pd.to_datetime(paginated_data[col]).dt.strftime("%Y-%m-%d")
...
def render_stock_forecast_chart(data, msku):
    ...
    row = data.iloc[0]
    # ========== 调整1：兼容非日期类型的时间 ==========
    start_date = pd.to_datetime(row["时间"]) if not pd.isna(row["时间"]) else pd.Timestamp.now()
...
def render_product_detail_chart(df, msku):
    ...
    product_data = df[df["MSKU"] == str(msku)].sort_values("时间")
    ...
    for _, row in product_data.iterrows():
        # 时间兼容（转换为datetime）
        record_date = pd.to_datetime(row["时间"]) if not pd.isna(row["时间"]) else pd.Timestamp.now()
    ...
    # 图表布局优化
    fig.update_layout(
        title=f"{msku} 不同时间的库存预测对比（含分阶段系数）",
        xaxis_title="日期",
        yaxis_title="剩余库存",
        plot_bgcolor="#f8f9fa",
        height=400,
...
def main():
    ...
    with st.sidebar:
        ...
        st.subheader("数据加载中...")
        try:
            data_url = "https://raw.githubusercontent.com/Jane-zzz-123/streamlit-dashboard/main/kucunfenxibymonth.xlsx"
    ...
    if current_data is not None and not current_data.empty:
        ...
        if selected_store:
            ...
            def get_store_last_week_metrics():
                ...
                current_date = pd.to_datetime(store_current_data["时间"].iloc[0])
                ...
                if prev_data_full is not None and not prev_data_full.empty:
                    ...
                    prev_data_filtered['时间'] = pd.to_datetime(prev_data_filtered['时间'])
                    last_week_data = prev_data_filtered[
                        (prev_data_filtered['时间'] >= last_week_start) &
                        (prev_data_filtered['时间'] <= last_week_end)
                        ]
                    # 过滤年份品（只统计年份品）
