...
def calc_metrics(df_curr, df_prev, risk_name):
    ...
    unsale_stock_curr = curr_data[curr_data["滞销风险等级"].isin(risk_list)]["总库存"].sum()
    unsale_stock_prev = prev_data[prev_data["滞销风险等级"].isin(risk_list)]["总库存"].sum()
    unsale_stock_diff = unsale_stock_curr - unsale_stock_prev

    unsale_amt_curr = curr_data[curr_data["滞销风险等级"].isin(risk_list)]["总滞销金额"].sum()
    unsale_amt_prev = prev_data[prev_data["滞销风险等级"].isin(risk_list)]["总滞销金额"].sum()
    unsale_amt_diff = unsale_amt_curr - unsale_amt_prev

    if risk_name in risk_list:
        usp = unsale_stock_curr / stock_curr if stock_curr !=0 else 0
        uap = unsale_amt_curr / amt_curr if amt_curr !=0 else 0
    else:
        usp = 0.0
        uap = 0.0
    ...
    return {
        ...
        "unsale_stock_curr":unsale_stock_curr,"unsale_stock_prev":unsale_stock_prev,"unsale_stock_diff":unsale_stock_diff,"unsale_stock_pct":usp,
        "unsale_amt_curr":unsale_amt_curr,"unsale_amt_prev":unsale_amt_prev,"unsale_amt_diff":unsale_amt_diff,"unsale_amt_pct":uap
    }
...
for idx, config in enumerate(card_config):
    ...
    if config['title'] != "健康":
        ...
        unsale_stock_html = f"<div style='font-size:14px;margin-bottom:6px;color:#333333;'>滞销库存：{unsale_stock_curr:,}（占比：{unsale_stock_pct:.2%}）<span style='font-size:11px;color:{unsale_stock_color};font-weight:normal;'>（{unsale_stock_sign}，上月：{unsale_stock_prev:,}）</span></div>"
        ...
        unsale_amt_html = f"<div style='font-size:14px;color:#333333;'>滞销金额：{unsale_amt_curr:,}（占比：{unsale_amt_pct:.2%}）<span style='font-size:11px;color:{unsale_amt_color};font-weight:normal;'>（{unsale_amt_sign}，上月：{unsale_amt_prev:,}）</span></div>"

    # 渲染卡片
    with cols[idx]:
        html_content = f"""
        <div style="background-color:{config['bg_color']};padding:20px;border-radius:12px;line-height:2.2;margin-bottom:15px;">
            <div style="font-size:22px;font-weight:bold;text-align:center;margin-bottom:15px;color:#1a1a1a;">{config['title']}</div>
            <div style="font-size:18px;font-weight:bold;margin-bottom:8px;color:#1a1a1a;">SKU个数：{sku_curr:,}<span style="font-size:12px;color:{sku_color};font-weight:normal;">（{sku_sign}，上月：{sku_prev:,}）</span></div>
            <div style="font-size:14px;margin-bottom:6px;color:#333333;">总库存：{stock_curr:,}<span style="font-size:11px;color:{stock_color};font-weight:normal;">（{stock_sign}，上月：{stock_prev:,}）</span></div>
            {unsale_stock_html}
            <div style="font-size:14px;margin-bottom:6px;color:#333333;">总金额：{amt_curr:,}<span style="font-size:11px;color:{amt_color};font-weight:normal;">（{amt_sign}，上月：{amt_prev:,}）</span></div>
            {unsale_amt_html}
        </div>
        """
