...
def render_card_compact(title: str, metrics: Dict):
    ...
    sku_c, sku_s = fmt_diff(metrics["sku_diff"])
    ...
    parts = [
        ...
        f'<div style="font-size:18px;font-weight:bold;margin-bottom:8px;color:#1a1a1a;">SKU个数：{metrics["sku_curr"]:,.0f}<span style="font-size:12px;color:{sku_c};font-weight:normal;">（{sku_s}，上月：{metrics["sku_prev"]:,.0f}）</span></div>',
        f'<div style="font-size:14px;margin-bottom:6px;color:#333333;">总库存：{metrics["stock_curr"]:,.0f}<span style="font-size:11px;color:{stk_c};font-weight:normal;">（{stk_s}，上月：{metrics["stock_prev"]:,.0f}）</span></div>',
    ]
...
for idx, title in enumerate(card_titles):
    ...
    with cols[idx]:
        render_card_compact(title, metrics)